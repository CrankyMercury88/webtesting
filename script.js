...
function updateHeroTextOpacity() {
    ...
    const fadeStartPoint = window.innerHeight * 0.75; // Start fading at 75% up the viewport
    const fadeEndPoint = window.innerHeight * 0.25; // Fully transparent at 25% up
    ...
    heroTexts.forEach(text => {
        ...
        if (elementTop > fadeStartPoint) {
            ...
            // Above fade end point - fully transparent
            text.style.opacity = '0';
        } else {
            // In the fade zone - calculate opacity
            const fadeRange = fadeStartPoint - fadeEndPoint;
            const distanceFromStart = fadeStartPoint - elementTop;
            const opacity = 1 - (distanceFromStart / fadeRange);
...
// Run on load
window.addEventListener('load', updateHeroTextOpacity);
